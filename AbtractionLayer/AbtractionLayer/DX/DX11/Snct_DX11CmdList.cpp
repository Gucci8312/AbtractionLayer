...
void SnctDX11CmdList::SetRasterRizerState(ID3D11RasterizerState* raseterRizeState)
{
	m_pContext->RSSetState(raseterRizeState);
}

void SnctDX11CmdList::SetViewPort(float Width, float Height, float MinDepth, float MaxDepth)
{
...
void SnctDX11CmdList::ClearRTV(ISnctDXRTV* Descriptors, UINT NumRects, RECT* pRects)
{
...
void SnctDX11CmdList::SetRTV(UINT NumDescriptors, ISnctDXRTV* Descriptors, ISnctDXDSV* DSHandle, bool SingleHandleToDescriptorRange)
{
	SnctDX11RTV* TempRTV = static_cast<SnctDX11RTV*>(Descriptors);
...
void SnctDX11CmdList::Close(bool RestoreDeferredContextState, ID3D11CommandList** cmdList)
{
